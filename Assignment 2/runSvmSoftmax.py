...
################################################################################
l_rate = [1e-6, 6e-6, 1e-7, 6e-7, 1e-8, 6e-8]
r_strength = [5e3, 5e4, 1e4, 7e4, 1e3, 7e3]


#iterate over learning rate and reg strength
for lr in l_rate:
    for reg in r_strength:

        #train using different lr and reg
        classifier = Svm(xTrain.shape[1], numClasses)
        classifier.train(xTrain, yTrain, lr=lr, reg=reg, iter=1500, verbose=False)

        #validation accuracy
        acc = classifier.calAccuracy(xVal, yVal)

        #update best Acc, Model, Parameters
        if acc > bestAcc or bestAcc == -1:
